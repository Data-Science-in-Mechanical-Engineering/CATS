...
class TrafficDataSource(DataSource):
    def __init__(self, context_length, prediction_length, dataset_base_path, name):
        super().__init__(name, context_length, prediction_length, 1, 1, Path(dataset_base_path) / "traffic")
        self.name = name
        
        # Load the single CSV file
        csv_file = self.dataset_base_path / "traffic.csv"
        if not csv_file.exists():
            raise FileNotFoundError(f"Dataset file not found: {csv_file}")
        
        # Load data and extract only the OT column
        df = pd.read_csv(csv_file)
        ot_data = df['0'].values.reshape(-1, 1)  # Reshape to be 2D (timesteps, 1 feature)
...
if __name__ == "__main__":
    ...
    csv_file = "/data/datasets/traffic/traffic.csv"
    ...
    plt.plot(df.iloc[:, 1])
    plt.title("First Column of Traffic Data")
    plt.xlabel("Index")
    plt.ylabel("Value")
    plt.show()