...
if sys.version_info >= (3, 7):
    Packet = namedtuple(
        "Packet",
        ["sender", "receiver", "slot", "flags", "iv", "cv"],
        defaults=[None, None, None, None, None, None],
    )
else:
    Packet = namedtuple(
        "Packet", ["sender", "receiver", "slot", "flags", "iv", "cv"]
    )
...
class MyMainForm(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.colors = matplotlib.colormaps["tab20"].colors
        ...
        for slot in slots:
            try:
                for packet in self.packet_log[rnd][slot]["packets"]:
                    ...
                    if txNode not in packetcolors:
                        packetcolors[txNode] = colors.to_hex(
                            self.colors[
                                len(packetcolors.keys()) % self.numColors
                            ]
                        )
                    ...
                    link = Link(
                        txNode, rxNode, packetcolors[txNode], isRequest
                    )
            ...
            if slot == slots[-1]:
                try:
                    for node, density in self.packet_log[rnd][slot][
                        "neighbors"
                    ].items():
                        if density == "-":
                    ...
                    for node, discoveryExit in self.packet_log[rnd][slot][
                        "discoveryExit"
                    ].items():
                        if discoveryExit:
                    ...
                    for node, shutdown in self.packet_log[rnd][slot][
                        "shutdown"
                    ].items():
                        if shutdown:
                    ...
                    for node, rank in self.packet_log[rnd][slot][
                        "rank"
                    ].items():
                        self.nodes[node].rank = rank

                    for node, rankUp in self.packet_log[rnd][slot][
                        "rankUp"
                    ].items():
                    ...
                    for node, txp in self.packet_log[rnd][slot]["txp"].items():
                        if txp == "-":
                            ...
                            self.nodes[node].txp = int(
                                round(txp / 65535, 2) * 100
                            )

                    for node, rows in self.packet_log[rnd][slot][
                        "rows"
                    ].items():
    ...
    def loadLog(self):
        ...
        logID_to_phyID = {
            v: k for (k, v) in self.mlp.exp_config["NODE_MAPPING"]
        }
        ...
        with open(self.mlp.log_formatted, "r", encoding="utf-8") as log:
            ...
            for line in lines:
                ...
                if cur_node is not None and cur_node != nodeid:
                    cur_rnd = None
                    cur_slot = None
                cur_node = nodeid
                ...
                # Current round
                res = re.search(
                    r"(starting round|preparing round) (?P<round>\d+)", msg
                )
                if res:
                    cur_rnd = int(res.group("round"))
                    if cur_rnd not in self.packet_log:
                        self.packet_log[cur_rnd] = {}
                        for slot in range(
                            0, self.mlp.exp_config["MX_ROUND_LENGTH"] + 6
                        ):  # some offset
                            self.packet_log[cur_rnd][slot] = {
                                "rank": {},
                                "txp": {},
                                "packets": [],
                                "rankUp": {},
                ...
                # Packets
                res = re.search(
                    r"(?P<slot>[0-9a-f]+) - (?P<sender>[0-9a-f]+) - (?P<flags>[0-9a-f]+) - (?P<iv>[0-9a-f]+) - (?P<cv>[0-9a-f]+)",
                    msg,
                )
                if res:
                    ...
                    try:
                        ...
                        sender = logID_to_phyID[
                            int(res.group("sender")[1:], 16)
                        ]
                        self.packet_log[cur_rnd][slot]["packets"].append(
                            Packet(sender, nodeid, slot, flags, iv, cv)
                        )
                        self.packet_log[cur_rnd][slot]["neighbors"][nodeid] = (
                            set([sender])
                        )
                    except KeyError as ke:
                        logger.error(
                            f'KeyError {ke}, check the log!\nMsg: {msg}\n\tsender {int(res.group("sender")[1:], 16)}\n\tcur_rnd {cur_rnd}\n\tslot {slot}'
                        )
                        continue
                ...
                if "rank_up_row=[" in msg:
                    ...
                    for rowCnt, row in enumerate(rankUpRows, 1):
                        for e, s in enumerate(
                            range(
                                idx, self.mlp.exp_config["MX_ROUND_LENGTH"] + 6
                            ),
                            1,
                        ):
                            if nodeid in self.packet_log[cur_rnd][s]["rankUp"]:
                                if self.packet_log[cur_rnd][s]["rankUp"][
                                    nodeid
                                ]:
                                    if (
                                        nodeid
                                        in self.packet_log[cur_rnd][s]["rows"]
                                    ):
                                        self.packet_log[cur_rnd][s]["rows"][
                                            nodeid
                                        ].append(row)
                                    else:
                                        self.packet_log[cur_rnd][s]["rows"][
                                    ...
                                    if (s == 0) and (
                                        rowCnt
                                        < self.packet_log[cur_rnd][s]["rank"][
                                            nodeid
                                        ]
                                    ):
                                        idx = 0
                ...
                # Discovery phase
                res = re.search(
                    r"discovery_exit_slot: (?P<discoveryExit>\d+)", msg
                )
                if res:
                    discoveryExit = int(res.group("discoveryExit"))
                    self.packet_log[cur_rnd][discoveryExit]["discoveryExit"][
                        nodeid
                    ] = True
                    continue
                ...
                if res:
                    ...
                    self.packet_log[cur_rnd][slotOff]["shutdown"][nodeid] = (
                        True
                    )
                    continue
                ...
                res2 = re.search(
                    r"RADIO_IRQHandler_\s*\(re\)synchronized to slot (?P<slot>\d+)",
                    msg,
                )
                if res or res2:
                    if res:
            ...
            for rnd in all_rounds:
                ...
                for node in all_nodes:
                    ...
                    for slot in all_slots:
                        ...
                        if node not in self.packet_log[rnd][slot]["txp"]:
                            self.packet_log[rnd][slot]["txp"][node] = "-"

                        if node not in self.packet_log[rnd][slot]["rankUp"]:
                            self.packet_log[rnd][slot]["rankUp"][node] = False

                        # add up nodes from received packets to neighbor set
                        if node not in self.packet_log[rnd][slot]["neighbors"]:
                            self.packet_log[rnd][slot]["neighbors"][node] = (
                                neighbors.copy()
                            )
                        else:
                            neighbors |= self.packet_log[rnd][slot][
                                "neighbors"
                            ][node]
                            self.packet_log[rnd][slot]["neighbors"][node] = (
                                neighbors.copy()
                        ...
                        if node not in self.packet_log[rnd][slot]["rows"]:
                            self.packet_log[rnd][slot]["rows"][node] = (
                                rows.copy()
                            )
                        else:
                            rows.extend(
                        ...
                        if node not in self.packet_log[rnd][slot]["rank"]:
                            if last_rank is None:
                                # TODO: Initial rank is based on message distribution.
                                self.packet_log[rnd][slot]["rank"][node] = 0
                            else:
                                self.packet_log[rnd][slot]["rank"][node] = (
                                    last_rank
                                )
                        else:
                            last_rank = self.packet_log[rnd][slot]["rank"][
                                node
                            ]
                        ...
                        if (
                            node
                            not in self.packet_log[rnd][slot]["discoveryExit"]
                        ):
                            self.packet_log[rnd][slot]["discoveryExit"][
                                node
                            ] = discoveryExit
                        else:
                            if not self.packet_log[rnd][slot]["discoveryExit"][
                                node
                            ]:
                                logger.error(
                                    "discoveryExit value must be True at this point!"
                                )
                            discoveryExit = True

                        if node not in self.packet_log[rnd][slot]["shutdown"]:
                            self.packet_log[rnd][slot]["shutdown"][node] = (
                                shutdown
                            )
                        else:
                            if not self.packet_log[rnd][slot]["shutdown"][
                                node
                            ]:
                                logger.error(
                                    "shutdown value must be True at this point!"
                                )
                            shutdown = True
