...
# Include to log into a file.
# file_handler	= logging.FileHandler('last_execution.ansi', mode='w', encoding='utf-8')
# formatter		= logging.Formatter(fmt='%(asctime)s %(filename)-15.15s:%(lineno)-5d %(levelname)-8s %(message)s', datefmt='%H:%M:%S')
...
class MixerLogParser:
    def __init__(self, basepath, files=None, description=None, testbed=None):
        logger.info(
            f"Initializing MixerLogParser for experiment {Path(basepath).name}"
        )
        ...
        if files:
            ...
            logger.critical(f"{self.log_formatted} does not exist.")
            sys.exit()
        ...
        if description:
            ...
            logger.critical(f"{self.log_config} does not exist.")
            sys.exit()
    ...
    def _format_log(self):
        if self.log_formatted.exists():
            logger.info(
                f"{self.log_formatted} already exists, skipping this step."
            )
        else:
            logger.info(f"Creating {self.log_formatted}")

            # TODO: automatically detect testbed

            if not self.testbed:
                logger.critical("Testbed format is not provided")
                sys.exit()

            elif self.testbed == "local":
                logger.info("Parsing experiment log of local testbed")
                ...
                for f in self.files:
                    ...
                    with open(f, "r", encoding="iso-8859-1") as log:
                        ...
                        for line in lines:
                            ...
                            if "# ID:" == line[0:5]:
                                xid = int(line[5:].split()[0])
                                #xid = int(line.split()[1].split(":")[1])
                                if phyid >= 0:
                                    xid_phy_offset = phyid - xid
                                    break
                        guess_phyid = True if phyid < 0 else False
                        if guess_phyid:
                            if xid < 0:
                                phyid = logid = min(nodelog) - 1
                                logger.warning(
                                    f"unable to detect node ID in file {f}, assigned ID {phyid}"
                                )
                            else:
                                phyid = xid + xid_phy_offset
                                logid = xid
                        if phyid not in nodelog:
                            nodelog[phyid] = []
#                             found_node_mapping[nodeid] = False
                        else:
                            logger.warning(
                                f"found multiple files with node ID {phyid}, concatenating {f}"
                            )
                        if guess_phyid:
                            nodelog[phyid].append(f"mapped physical node {phyid} to logical id {logid}\n")
                        for line in lines:
                            nodelog[phyid].append(line)
#                             if "mapped physical node" in line:
#                                 found_node_mapping[nodeid] = True

#                 # add missing node ID mapping infos (if not found in files)
#                 for nodeid in nodelog:
#                     if not found_node_mapping[nodeid]:
#                         # If nodeid 0 has been found then we assume that nodeid refers to
#                         # logical IDs (as it should be), otherwise we assume that it represents
#                         # physical IDs. In the latter case we can map one-to-one, in the former
#                         # we avoid guessing valid physical IDs and instead use negative values
#                         # to explicitly mark this case and avoid conflicts with other mappings
#                         phy_id = nodeid if 0 not in nodelog else -abs(nodeid)
#                         nodelog[nodeid].append(f"mapped physical node {phy_id} to logical id {nodeid}\n")
                ...
                with open(self.log_formatted, "w", encoding="utf-8") as out:
                    for node in sorted(nodelog.keys()):
                        for line in nodelog[node]:
                            if "\\x" in repr(line):
                                logger.debug(f"Skipping line: {repr(line)}")
                                continue
                            timestamp = 0
                            msg = line
                ...
                logger.info("Parsing experiment log of graz testbed")
                # Gather all lines of each node.
                nodelog = {}
                for f in self.files:
                    # potential encodings: utf-8, iso-8859-1, ascii, ...
                ...
                logger.info("Parsing experiment log of flocklab testbed")
                # Gather all lines of each node.
                nodelog = {}
                for f in self.files:
                    # potential encodings: utf-8, iso-8859-1, ascii, ...
                    with open(f, "r", encoding="utf-8") as log:
                        ...
                        for line in lines:
                            ...
                            if (
                                ...
                                in line
                                or "timestamp,observer_id,node_id,direction,output"
                                in line
                            ):
                                continue
                            nodeid = int(line.split(",")[1])
                            if nodeid not in nodelog:
                                nodelog[nodeid] = []
                            nodelog[nodeid].append(line)
    ...
    def _extract_config(self):
        if self.log_config.exists():
            logger.info(
                f"{self.log_config} already exists, skipping this step"
            )
        else:
            logger.info(f"Creating {self.log_config}")

            cfg = {
                "LOG": self.log_formatted,
            ...
            with open(self.log_formatted, "r", encoding="utf-8") as log:
                ...
                for line in lines:
                    ...
                    if len(parameter) > 0:
                        ...
                        cfg[parameter] = int(
                            line.split(parameter)[1].split("=")[1]
                        )
    ...
    def filter_log(self, lines_of_interest):
        ...
        if self.log_filtered.exists():
            logger.info(
                f"{self.log_filtered} already exists, skipping this step."
            )
        else:
            logger.info(f"Creating {self.log_filtered}")
...
def main():
    ...
    parser.add_argument(
        "files", help="pattern of file(s) ending, e.g. '*.txt'"
    )
    parser.add_argument(
        "description", help="experiment description (in quotes)"
    )
    parser.add_argument(
        "testbed",
        help="the parser needs to know the testbed for correct formatting",
